...
RhoGEF.pos.time <- function(total.time, numCS, Plk.time, retention, relative.velocity) {
		...
		#retention=the retention rate of unbound CS at plus-end tips prior to polo phosphorylation
		#relative.velocity=how much faster/slower CS-RhoGEF transport is than unbound CS walking along microtubule
		...
		for (i in seq_along(time)) { #for every timepoint
			if (i < Plk.time) { #before polo phosphorylation
				...
				if (CS.position>=MT.length) { #if CS reaches plus-end tip
					CS.ret <- sample(0:1, prob=c((1-retention), retention))[1] #determines if unbound CS remains on the plus-end tip based on entered retention value
					if (CS.ret==1) {
						CS.position <- MT.length #unbound CS is retained on plus-end tip
					} else {
				...
				if (RhoGEF.CS==0) { #for any unbound CS molecule
				...
				if (RhoGEF.CS==1) {
					...
					CS.position <- CS.position+1 #unbound CS advances forward 1 position
				}
				...
				if (CS.position>=MT.length && RhoGEF.CS==0) { #for unbound CS at plus-end tip
					...
					if (CS.ret==1) {
						CS.position <- MT.length #unbound CS retained at plus-end tip
					} else {
				...
				if (RhoGEF.CS==1) { #for CS-RhoGEF complexes
					...
					RhoGEF.pos <- 0 #for unbound CS, no RhoGEF position on microtubule
				}
				rhogef.time <- append(rhogef.time, RhoGEF.t) #add current timepoint to output
...
test.null <- RhoGEF.pos.time(50,1000,25,0.0000001,1) #no unbound retention + no CS stimulation by RhoGEF
test.retain <- RhoGEF.pos.time(50,1000,25,0.89,1) #unbound retention + no CS stimulation by RhoGEF
test.retain_transport <- RhoGEF.pos.time(50,1000,25,0.89,(11/6)) #unbound retention + CS stimulation by RhoGEF
